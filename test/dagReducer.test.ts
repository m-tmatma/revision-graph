...
describe('reduceDag', () => {
  it('returns commits unchanged when collapseStraightRuns is disabled', () => {
    const commits = [commit('a', ['b']), commit('b', ['c']), commit('c', [])];

    const reduced = reduceDag(commits, { showAllTags: false, collapseStraightRuns: false });

    expect(reduced).toEqual(commits);
  });

  it('splices out a straight run of unreffed single-parent/single-child commits', () => {
    // a -> b -> c -> d, only a and d have refs. b and c should be elided,
    // with a's parent rewritten straight to d.
    const commits = [
    ...
    const reduced = reduceDag(commits, { showAllTags: true, collapseStraightRuns: true });
  ...
  it('treats tag refs as non-protecting when showAllTags is false', () => {
    ...
    const reduced = reduceDag(commits, { showAllTags: false, collapseStraightRuns: true });
